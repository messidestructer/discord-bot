    ...
    async def profile(self, interaction: discord.Interaction, member: discord.Member = None):
        ...
        if roblox_name:
            ...
            if roblox_id:
                ...
                embed.add_field(
                    name="Group Rank",
                    value=f"{rank_name or 'Not in group'}" + (f" (#{rank_num})" if rank_num else ""),
                    inline=True,
                )
                embed.add_field(
                    name="Roblox Profile",
            ...
            embed.add_field(name="Roblox", value="Not verified", inline=True)

        if ep_rec:
            embed.add_field(name="Total EP", value=str(ep_rec["ep"]), inline=True)
            embed.add_field(
                name="Last Active",
                value=ep_rec.get("last_updated", "Unknown")[:10],
                inline=True,
            )
        else:
            embed.add_field(name="EP", value="0", inline=True)
